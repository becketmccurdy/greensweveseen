...
export async function POST(request: NextRequest) {
  try {
    // Simple admin protection - in production you'd want proper auth
    ...
    for (const courseData of popularCourses) {
      try {
        ...
        if (!existingCourse) {
          ...
          seedCount++
          results.push({ status: 'added', course: courseData.name })
        } else {
          results.push({ status: 'skipped', course: courseData.name, reason: 'already exists' })
        }
      } catch (error) {
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully seeded ${seedCount} new courses!`,
      totalProcessed: popularCourses.length,
      seedCount,
      results
    })
